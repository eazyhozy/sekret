...
- If the env var matches a built-in registry entry, the name is suggested as a default (e.g. `OPENAI_API_KEY` → `openai (recommended)`). The user can accept by pressing Enter or type a custom name.
- Otherwise, the user must provide a name.
...
Import each key? (y: import / s: skip / q: quit)
  ...
  [1/3] OPENAI_API_KEY → openai (recommended)
        Import? [Y/s/q]: y
        Name (press Enter for "openai"): 
        Imported openai (OPENAI_API_KEY)

  [2/3] KAP_API_KEY → ? (unknown)
        Import? [Y/s/q]: y
        Name for KAP_API_KEY: kap
        Imported kap (KAP_API_KEY)

  [3/3] SLACK_WEBHOOK_SECRET → ? (unknown)
        Import? [Y/s/q]: s
        Skipped
...
Done. 2 imported, 1 skipped.
  ...
  Imported:
    ~/.zshrc:12     OPENAI_API_KEY (openai)
    ~/.zshrc:168    KAP_API_KEY (kap)
